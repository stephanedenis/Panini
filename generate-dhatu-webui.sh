#!/bin/bash
# Generate Dhātu Web UI components

cd /home/stephane/GitHub/Panini-FS

WEBUI_DIR="panini-fs-web-ui"

echo "Creating DhatuDashboard.tsx..."
cat > "$WEBUI_DIR/src/pages/DhatuDashboard.tsx" << 'EOFDASHBOARD'
...
const DhatuDashboard: React.FC = () => {
  ...
  const handleClassify = async () => {
    ...
    try {
      ...
      // Reload stats after classification
      await loadStats();
    } catch (error) {
...
echo "✅ DhatuDashboard.tsx created"

echo "Adding route to App.tsx..."
# Check if App.tsx already has Dhatu route
if grep -q "DhatuDashboard" "$WEBUI_DIR/src/App.tsx"; then
    echo "⚠️  DhatuDashboard route already exists"
else
    # Add import
    sed -i '/import.*from.*pages/a import DhatuDashboard from "./pages/DhatuDashboard";' "$WEBUI_DIR/src/App.tsx"
    # Add route (before closing Routes tag)
    sed -i '/<Route path="\/file-upload"/a \          <Route path="/dhatu" element={<DhatuDashboard />} />' "$WEBUI_DIR/src/App.tsx"
    echo "✅ Route added to App.tsx"
fi

echo ""
echo "🎉 Dhātu Web UI generated!"
echo ""
echo "Access at: http://localhost:5173/dhatu"
