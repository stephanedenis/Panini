#!/bin/bash
# Generate Dhātu API handlers

cd /home/stephane/GitHub/Panini-FS

echo "Creating dhatu_handlers.rs..."
cat > crates/panini-api/src/dhatu_handlers.rs << 'EOFHANDLERS'
//! Dhātu API Handlers
//!
...
pub async fn get_roots(
    State(state): State<Arc<DhatuState>>,
    ...
    let roots = state.classifier.get_roots(emotion);
...
pub async fn classify_content(
    ...
    let intensity = state.classifier.classify_content(&req.content);
...
echo "✅ dhatu_handlers.rs created"

echo "Updating routes.rs to add Dhātu endpoints..."

# Check if routes.rs exists
if [ -f "crates/panini-api/src/routes.rs" ]; then
    # Add module declaration if not present
    if ! grep -q "pub mod dhatu_handlers" crates/panini-api/src/routes.rs; then
        echo "Adding dhatu_handlers module..."
        sed -i '1i pub mod dhatu_handlers;' crates/panini-api/src/routes.rs
    fi
    
    echo "✅ routes.rs updated"
    echo "⚠️  Manual step: Add Dhātu routes to router in routes.rs"
else
    echo "⚠️  routes.rs not found - will need manual integration"
fi

echo ""
echo "🎉 Dhātu API handlers generated!"
echo ""
echo "Next steps:"
echo "1. Add to routes.rs router:"
echo "   .route(\"/api/dhatu/emotions\", get(dhatu_handlers::get_emotions))"
echo "   .route(\"/api/dhatu/roots/:emotion\", get(dhatu_handlers::get_roots))"
echo "   .route(\"/api/dhatu/classify\", post(dhatu_handlers::classify_content))"
echo "   .route(\"/api/dhatu/search\", get(dhatu_handlers::search_profiles))"
echo "   .route(\"/api/dhatu/stats\", get(dhatu_handlers::get_stats))"
echo "   .route(\"/api/dhatu/resonance\", post(dhatu_handlers::calculate_resonance))"
echo "2. Initialize DhatuState in main.rs"
